...
version: "3.0"
updated: "2026-02-07"
description: "Kiro CLI + tmux multi-agent parallel dev platform with sengoku military hierarchy"
...
**This is ONE procedure for ALL situations**: fresh start, compaction, session continuation, or any state where you see .kiro/steering/shogun-system.md. You cannot distinguish these cases, and you don't need to. **Always follow the same steps.**
...
3. **Read `memory/MEMORY.md`** (shogun only) — persistent cross-session memory. If file missing, skip. *Kiro CLI users: this file is also auto-loaded via Kiro CLI's memory feature.*
4. **Read your instructions file**: shogun→`instructions/generated/kiro-shogun.md`, karo→`instructions/generated/kiro-karo.md`, ashigaru→`instructions/generated/kiro-ashigaru.md`, gunshi→`instructions/generated/kiro-gunshi.md`. **NEVER SKIP** — even if a conversation summary exists. Summaries do NOT preserve persona, speech style, or forbidden actions.
...
Lightweight recovery using only .kiro/steering/shogun-system.md (auto-loaded). Do NOT read instructions/*.md (cost saving).
...
- Read your instructions file (shogun→`instructions/generated/kiro-shogun.md`, etc.)
...
**Always Read before Write/Edit.** Kiro CLI rejects Write/Edit on unread files.
...
Layer 4: Session context — volatile (.kiro/steering/shogun-system.md auto-loaded, instructions/*.md, lost on /clear)
```
